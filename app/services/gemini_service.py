...
class GeminiService:
    ...
    async def _craft_image_prompt(self) -> str:
        ...
        elif snapshot.light < LIGHT_NORMAL_LUX_UPPER_THRESHOLD:
            prompt.append("Use soft, natural indoor lighting.")
        else:
        ...
        elif snapshot.temperature <= TEMPERATURE_COMFORT_C_UPPER_THRESHOLD:
            prompt.append(
                "Keep a neutral, comfortable atmosphere in the image."
            )
        else:
            prompt.append("Add a warm, slightly hot atmosphere to the image.")
